#!/bin/sh
# Build Advanced Rogue 5.8 for the browser (Emscripten + Asyncify) into web/dist;
# deploy with web/deploy.sh. Run with sh (zsh doesn't split $SRCS).
set -e
cd "$(dirname "$0")/.."
OUT=web/dist
rm -rf "$OUT" && mkdir -p "$OUT"
SRCS=$(printf 'print:\n\t@echo $(CFILES)\n' | make -s -f Makefile -f - print)
...
cp web/index.html "$HOME/Games/rvip-tools/web/rvip-wm.js" web/arogue58.js port/tiles.png port/tiles-dawn.png "$OUT/"
