...
for f in ('blueberry candleberry caprifig dewberry elderberry gooseberry guanabana hagberry '
    ...
    EXTRA[f] = 'slime mold'
CLASS = ['barbarian', 'wizard', 'cleric', 'rogue', 'human']   # fighter magician cleric thief monster
...
def fuzzy(n, fallback):
    ...
    for c in (ALIAS.get(n), n, n.lower()):
        if c and c in idx:
            return idx[c]
    w = n.lower().split()
...
mon_tiles = [fuzzy(n, 'invisible monster') for n in table(MON_TABLE)]
mons = table(MON_TABLE)
WEAP = table('weaps'); ARMOR = table('armors'); MMAG = table('m_magic')
RELIC = table('rel_magic'); FOOD = table('foods')
...
out = ['/* generated by port/mktiles.py from the NetHack tiles - do not edit */\n',
       ...
       arr('weap_tile', [fuzzy(n, 'weapon / generic weapon') for n in WEAP] or [-1]),
       arr('armor_tile', [fuzzy(n, 'armor / generic armor') for n in ARMOR] or [-1]),
       arr('mm_tile', [fuzzy(n, 'tool / generic tool') for n in MMAG] or [-1]),
       arr('relic_tile', [fuzzy(n, 'amulet / generic amulet') for n in RELIC] or [-1]),
